...
<?php $__env->startSection('content'); ?>
    ...
    <script>
        ...
        function get_data(page){
            $.ajax({
                url:"<?php echo e(url('Application_Setup/get_hotels')); ?>?page="+page,
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"POST",
                dataType:"JSON",
                success:function (data) {
                    ...
                    for(i in data.data){
                        ...
                        htmlData+=' <a  class="btn btn-danger btn-xs" href="javascript:void(0)" onclick="del_rec(\''+data.data[i].id+'\', \'<?php echo e(url('Application_Setup/hotel')); ?>/'+data.data[i].id+'\')"><i class="fa fa-trash"></i> </a>';
                        htmlData+='</td>';
                        htmlData+='</tr>';
                    }
        ...
        function edit(id) {
            $("#new").modal();
            $.ajax({
                url: "<?php echo e(url('Application_Setup/hotel')); ?>/" + id + "/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success: function (data) {
