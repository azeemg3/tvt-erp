...
<?php $__env->startSection('content'); ?>
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="#">Umrah</a></li>
                            <li class="breadcrumb-item active">Umrah Trip List</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
    ...
    <!-- /.content-wrapper -->
    <?php echo $__env->make('agents.agent_booking.brn-modal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    ...
    <script>
        ...
        function get_data(page){
            $("#loader").show();
            $.ajax({
                url:"<?php echo e(url('agent_management/get_agent_umrah')); ?>?page="+page,
                ...
                success:function (data) {
                    ...
                    for(i in data.data){
                        ...
                        htmlData += '<td>'+data.data[i].dep_date+' '+data.data[i].arr_sector+'</td>';
                        htmlData += '<td>'+data.data[i].dep_date+' '+data.data[i].dep_sector+'</td>';
                        htmlData += '<td>'+data.data[i].totalPax+'</td>';
                        htmlData += '<td>'+data.data[i].conversion_rate+'</td>';
                        htmlData += '<td>'+(tt).toFixed(2)+'</td>';
                        htmlData += '<td class="status">'+(data.data[i].status==1?'Approved':'Pending')+'</td>';
                        htmlData += '<td class="noExl">';
                        htmlData += '<a  class="btn btn-warning btn-xs" href="<?php echo e(url('agent_management/agent_umrah')); ?>/' + data.data[i].id + '/edit"><i class="fa fa-edit"></i></a>';
                        htmlData += ' <a  class="btn btn-primary btn-xs" href="<?php echo e(url('agent_management/umrah_details')); ?>/' + data.data[i].id + '"><i class="fa fa-eye"></i> Detail</a>';
                        if(data.data[i].status==0) {
                            htmlData += ' <a href="javascirpt:void(0)" class="btn btn-success btn-xs" onclick="approve_modal('+data.data[i].id+')">Approve</a>';
                        }else{
                            htmlData += ' <a target="_blank" href="<?php echo e(url('agent_management/agent_umrah')); ?>/' + data.data[i].id + '""  class="btn btn-success btn-xs"><i class="fa fa-eye"> Voucher</a>';
                        }
                        htmlData += '</td>';
                        htmlData += '</tr>';
                    }
        ...
        function approve_voucher() {
            ...
            $.ajax({
                url: "<?php echo e(url('agent_management/approve_uv')); ?>",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: "POST",
                data: $("#form").serialize(),
                success: function (data) {
                    ...
                    $("#" + id).find('.btn-success').hide();
                    $("#form input[name~='id']").val(id);
                    $("#brn-modal").modal('hide');
                }
            });
        }
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp8.2\htdocs\uotrips\resources\views/agents/agent_booking/index.blade.php ENDPATH**/ ?>