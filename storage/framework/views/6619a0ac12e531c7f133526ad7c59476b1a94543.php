...
<script>
    ...
    function get_lead_conversation() {
        $.ajax({
            url:"<?php echo e(url('lms/get_lead_conversation')); ?>/<?php echo e($result[0]->id); ?>",
    ...
    function lead_conversation(ob) {
        ...
        $.ajax({
            url:"<?php echo e(url('lms/lead_conversation')); ?>",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type:"POST",
            dataType:"JSON",
    ...
    function get_lead_details() {
        $("#ledger-modal").modal();
        $.ajax({
            url:"<?php echo e(url('lms/get_lead_details/')); ?>/<?php echo e($result[0]->id); ?>",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"JSON",
            success:function (data) {
    ...
    function creae_ledger() {
        $.ajax({
            url:"<?php echo e(url('lms/lead_ledger/')); ?>",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"JSON",
            type:"POST",
...
</script><?php /**PATH E:\new-projects\htdocs\uotrips\resources\views/Lms/js_func.blade.php ENDPATH**/ ?>