...
@section('content')
    ...
    <!-- /.content-wrapper -->
    @include('Rate_setup.visa_rate.modal')
    ...
    <script>
        ...
        function get_data(page){
            $("#loader").show();
            $.ajax({
                url:"{{ url('Application_Setup/Rate_Setup/get_visa_rate') }}?page="+page,
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"POST",
                dataType:"JSON",
                ...
                success:function (data) {
                    ...
                    for(i in data.data){
                        ...
                        htmlData+=' <a  class="btn btn-danger btn-xs" href="javascript:void(0)" onclick="del_rec(\''+data.data[i].id+'\', \'{{ url('Application_Setup/Rate_Setup/visa_rate') }}/'+data.data[i].id+'\')"><i class="fa fa-trash"></i> </a>';
                        htmlData+='</td>';
                        htmlData+='</tr>';
                    }
        ...
        function edit(id) {
            $("#new").modal();
            $.ajax({
                url: "{{ url('Application_Setup/Rate_Setup/visa_rate') }}/" + id + "/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success: function (data) {
...
//            var sale=$(g).closest('.row').find('.sale_price').val();
        ...
        function approved_rate() {
            ...
            $.ajax({
                url:"{{ url('Application_Setup/Rate_Setup/approve_visa_rate') }}/"+id,
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"GET",
                dataType:"JSON",
                success:function (data) {
