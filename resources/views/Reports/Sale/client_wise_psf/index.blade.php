...
@section('content')
    <style>
        ...
        #table2excel .section-total th {
            font-weight: bold;
            background-color: #f1f3f5;
            border: 1px solid #dee2e6;
        }
        #table2excel .grand-total td,
        #table2excel .grand-total th {
            font-weight: bold;
            background-color: #e9ecef;
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Client Wise PSF</li>
                        </ol>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            ...
                            <div id="report-area">
                                ...
                                <div class="sale-report-header report-show">
                                    ...
                                    <div class="sale-report-title">Client Wise PSF Report</div>
    ...
    <script>
        ...
        function rowHtml(row) {
            ...
            html += '<td class="text-center">' + (row.ticket_no || '') + '</td>';
            html += '<td>' + (row.pax_name || '').toUpperCase() + '</td>';
            html += '<td class="text-center">' + (row.sector || '').toUpperCase() + '</td>';
            html += '<td class="text-right">' + formatNumber(row.fare) + '</td>';
            html += '<td class="text-right">' + formatNumber(row.psf) + '</td>';
            html += '<td class="text-right">' + formatNumber(row.psfp) + '%</td>';
        ...
        function sectionTotalHtml(totals) {
            ...
            html += '<td class="text-right">' + formatNumber(totals.fare) + '</td>';
            html += '<td class="text-right">' + formatNumber(totals.psf) + '</td>';
        ...
        function grandTotalHtml(totals) {
            var html = '<tr class="grand-total">';
            html += '<td colspan="5"></td>';
            html += '<th class="text-right">' + formatNumber(totals.fare) + '</th>';
            html += '<th class="text-right">' + formatNumber(totals.psf) + '</th>';
            html += '<th class="text-right"></th>';
            html += '<th class="text-right">' + formatNumber(totals.receiveable) + '</th>';
            html += '</tr>';
            return html;
        }
        ...
        function get_data() {
            ...
            $.ajax({
                url: "{{ url('reports/sale/get_client_wise_psf') }}",
                ...
                success: function (data) {
                    ...
                    if (!data.sections || data.sections.length === 0) {
                        html = '<tr class="empty-row"><td colspan="9">No records found for the selected criteria.</td></tr>';
                    } else {
                        for (var s in data.sections) {
                            var section = data.sections[s];
                            html += '<tr class="group-title"><td colspan="9">' + section.label + '</td></tr>';

                            if (section.rows.length === 0) {
                                html += '<tr class="empty-row"><td colspan="9">No records found.</td></tr>';
                            } else {
                        ...
                        if (data.grand_total) {
                            html += grandTotalHtml(data.grand_total);
                        }
                    }
        ...
        function emailReport() {
            var subject = encodeURIComponent('Client Wise PSF Report - {{ $company->name }}');
            var body = encodeURIComponent('Please find the Client Wise PSF report.\n\nFrom: ' + $('#display_from').text() + '\nTo: ' + $('#display_to').text());
        ...
        jq(document).ready(function () {
            jq(".exportToExcel").click(function () {
                jq("#table2excel").table2excel({
                    exclude: ".noExl",
                    name: "Client Wise PSF",
                    filename: "client_wise_psf_" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
                    fileext: ".xls",
                    exclude_img: true,
                    exclude_links: true,
                    exclude_inputs: true,
            ...
            jq(".exportToWord").click(function () {
                ...
                link.download = 'client_wise_psf_' + new Date().toISOString().slice(0, 10) + '.doc';
                link.click();
            });
        });
    </script>
