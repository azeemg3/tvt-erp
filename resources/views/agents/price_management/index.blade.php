...
@section('content')
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">Agent Managment</li>
                            <li class="breadcrumb-item active">Agent Price List</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
    ...
    <script>
        ...
        function get_data(page){
            $("#loader").show();
            $.ajax({
                url:"{{ url('agent_management/get_agents') }}?page="+page,
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"POST",
                dataType:"JSON",
                success:function (data) {
        ...
        function edit(id) {
            $("#new").modal();
            $.ajax({
                url: "{{ url('Accounts/agent') }}/" + id + "/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success: function (data) {
