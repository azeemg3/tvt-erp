...
@section('content')
    ...
    <!-- /.content-wrapper -->
    @include('agents.agent_booking.brn-modal')
    ...
    <script>
        ...
        function get_data(page){
            $("#loader").show();
            $.ajax({
                url:"{{ url('agent_management/get_agent_umrah') }}?page="+page,
                ...
                success:function (data) {
                    ...
                    for(i in data.data){
                        ...
                        htmlData += '<a  class="btn btn-warning btn-xs" href="{{ url('agent_management/agent_umrah') }}/' + data.data[i].id + '/edit"><i class="fa fa-edit"></i></a>';
                        htmlData += ' <a  class="btn btn-primary btn-xs" href="{{ url('agent_management/umrah_details') }}/' + data.data[i].id + '"><i class="fa fa-eye"></i> Detail</a>';
                        if(data.data[i].status==0) {
                            htmlData += ' <a href="javascirpt:void(0)" class="btn btn-success btn-xs" onclick="approve_modal('+data.data[i].id+')">Approve</a>';
                        }else{
                            htmlData += ' <a target="_blank" href="{{ url('agent_management/agent_umrah') }}/' + data.data[i].id + '""  class="btn btn-success btn-xs"><i class="fa fa-eye"> Voucher</a>';
                        }
                        htmlData += '</td>';
                        htmlData += '</tr>';
                    }
        ...
        function approve_voucher() {
            ...
            $.ajax({
                url: "{{ url('agent_management/approve_uv') }}",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: "POST",
                data: $("#form").serialize(),
                success: function (data) {
