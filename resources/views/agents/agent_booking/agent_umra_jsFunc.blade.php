<script type="text/javascript">
    ...
    function ground_service(g) {
        ...
        $.ajax({
            url:"{{ url('agent_management/get_ground_handleservices') }}/"+g,
...
//
    ...
    function save_umrah_rec(arg) {
        ...
        $.ajax({
            url:"{{ route('agent_umrah.store') }}?arg="+arg,
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type:"POST",
            dataType:"JSON",
            data:$.param(formData),
            success:function (data) {
                ...
                window.location.href="{{ url('agent_management/umrah_details') }}/"+data.id;
    ...
    function update_umrah_rec(arg) {
        ...
        $("")
        $.ajax({
            url:"{{ url('agent_management/agent_umrah') }}/"+arg,
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type:"PUT",
            dataType:"JSON",
            data:$.param(formData),
    ...
    $('#pax-form').submit(function (e) {
        ...
        $.ajax({
            url: "{{ url('agent_management/save_umrah_pax') }}",
            type: "POST",
            dataType: "JSON",
            data: formData,
            contentType: false,
    ...
    function edit_pax(id) {
        $("#new-pax").modal();
        $.ajax({
            url: "{{ url('agent_management/edit_upax') }}/"+id,
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            success: function (data) {
...
//                $(".visa_cost").val(window.btoa(data.visa_cost));
    ...
    $(document).on("change",".nights",function () {
        ...
        if(arrangement==1) {
            $.ajax({
                url: "{{ url('agent_management/fetch_hotel_rate') }}",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: "POST",
...
//            var nights = $(g).closest('.row').find('.nights').val();
    ...
    function more_transport(g) {
        ...
        <select class="form-control form-control-sm select2 to_city" name="to_city[]">\
            <option value="">Enter City Name</option>\
            {!! App\Models\Crm\UmrahTransportCity::dropdown() !!}\
        </select>\
        </div>\
        </div><!--col-->\
        <div class="col-md-2">\
    ...
    function transport_rate(g) {
        ...
        if(arrangement==1 && transport_include_visa==false && transport_check==true) {
            $.ajax({
                url: "{{ url('agent_management/fetch_transport_rate') }}",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: "POST",
    ...
    function visa_rate(g) {
        ...
        var agentID=$("#edit_agentID").val();
        $.ajax({
            url: "{{ url('agent_management/fetch_visa_rate') }}/"+type+"/"+agentID,
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type:"GET",
            success: function (data) {
                if(data.agent_price[0]==2){
    ...
    function fetch_agent_visitors(g) {
        ...
        $.ajax({
            url:"{{ url('agent_management/fetch_agent_visitors') }}/"+GID,
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type:"GET",
            success:function (data) {
                htmlData = '';
    ...
    function assign_visitor() {
        ...
        $.ajax({
            url:"{{ url('agent_management/assigned_visitors') }}",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type:"POST",
    ...
    function fetch_agent_groups(id) {
        $.ajax({
            url:"{{ url('agent_management/fetch_agent_group') }}/"+id,
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type:"GET",
            success:function (data) {
                var htmlData='';
    ...
    function get_agnt_hotel(g) {
        ...
        let citID=$(g).val();
        $.ajax({
            url:"{{ url('agent_management/get_agent_hotel') }}/"+agentID+"/"+citID,
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type:"GET",
            success:function (data) {
...
@include('agents.agent_booking.search-option-js')